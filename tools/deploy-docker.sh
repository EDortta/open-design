...
while (( SECONDS < deadline )); do
  ...
  echo "    bridge: $bridge_state | open-design: $app_state"
  if [[ "$bridge_state" == "running healthy" && "$app_state" == "running healthy" ]]; then break; fi
  if [[ "$bridge_state" == exited* || "$bridge_state" == restarting* || "$bridge_state" == dead* || "$bridge_state" == missing* ]]; then echo "ERRO: bridge não estabilizou: $bridge_state" >&2; false; fi
  if [[ "$app_state" == exited* || "$app_state" == dead* ]]; then echo "ERRO: OpenDesign não estabilizou: $app_state" >&2; false; fi
  sleep 3
done
